...
This guide provides information for developers who want to contribute to or modify the AMD Network Operator.
...
- Modify the registry-related variables in the `Makefile`:
  ...
  - `IMAGE_NAME`: Set to `rocm/network-operator`
...
- Build and push the AMD Network Operator image:
...
```bash
make e2e NETWORK_OPERATOR_CHART="path to helm chart"
```
