...
func NewNetworkConfigConditionMgr() ConditionUpdater {
	return &ConditionManager{}
}

func (cm *ConditionManager) SetReadyCondition(cr any, status metav1.ConditionStatus, reason, message string) {
	nwConfig := cr.(*amdv1alpha1.NetworkConfig)
	setCondition(nwConfig, metav1.Condition{
...
func (cm *ConditionManager) GetReadyCondition(cr any) *metav1.Condition {
	nwConfig := cr.(*amdv1alpha1.NetworkConfig)
	return findCondition(nwConfig.Status.Conditions, ConditionTypeReady)
}
...
func (cm *ConditionManager) DeleteReadyCondition(cr any) {
	nwConfig := cr.(*amdv1alpha1.NetworkConfig)
	deleteCondition(&nwConfig.Status.Conditions, ConditionTypeReady)
}

func (cm *ConditionManager) DeleteErrorCondition(cr any) {
	nwConfig := cr.(*amdv1alpha1.NetworkConfig)
	deleteCondition(&nwConfig.Status.Conditions, ConditionTypeError)
}

func setCondition(nwConfig *amdv1alpha1.NetworkConfig, newCondition metav1.Condition) {
	existingCondition := findCondition(nwConfig.Status.Conditions, newCondition.Type)
	...
	if existingCondition != nil {
		...
		*existingCondition = newCondition
		...
		nwConfig.Status.Conditions = append(nwConfig.Status.Conditions, newCondition)
	}
}
